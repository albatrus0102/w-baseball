...
**2026-09-05 추가.** 위 재현성은 지금까지 Windows 한 기계에서만 확인됐다 —
리눅스 CI 러너에서도 같은 픽셀이 나오는지는 아무도 재 본 적이 없었다. 폰트
파일이 같아도 래스터화(FreeType 버전, 힌팅, 서브픽셀 처리)는 OS 마다 다를 수
있어 보장되지 않는다. `.github/workflows/ci.yml` 에 `continue-on-error: true`
로 묶은 별도 단계(`golden-linux-check`)를 추가해 **리눅스에서 골든 31장을
비교 모드로 돌리기 시작**했다 — 기존 `--exclude-tags screenshots` 테스트
단계는 그대로 두고, 이 단계는 실패해도 job 을 막지 않는다. 어긋나는 골든이
있으면 `test/screenshots/failures/` 를 `golden-screenshot-failures` 아티팩트로
올린다. **이 절의 마지막 두 문단("게이트로 승격할지는 별도 결정 사항")은 아직
바뀌지 않았다** — 이번 변경은 그 결정에 필요한 리눅스 측정치를 얻는 것이지,
결정 자체가 아니다. 측정 결과는 CI 실행 로그와 아티팩트에 남는다.
